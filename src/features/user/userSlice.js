...
export const uploadUserPhoto = createAsyncThunk(
  ...
  async ({ userId, photoData }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user
      return await userService.uploadUserPhoto(userId, photoData, token)
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(uploadUserPhoto.fulfilled, (state, action) => {
        state.isLoading = false
        state.isSuccess = true
        state.profil = { ...state.profil, photo: action.payload } // Mise à jour de la photo dans le profil
      })
